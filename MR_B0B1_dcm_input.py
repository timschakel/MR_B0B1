...
# -*- coding: utf-8 -*-
"""
Created on Fri Aug 12 12:57:51 2022

@author: tschakel
Create an alternative prepareInput function with other RescaleSlope/Intercept dicomtags
(for older Philips data)
...
#import QCMR_pydicom_series as dcmseries
from wad_qc.modulelibs.wadwrapper_lib import pydicom_series as dcmseries
import numpy as np
import os
...
def prepareEnhancedInput(filename, headers_only, do_transpose=True, logTag="[prepareEnhancedInput] "):
    ...
    keymapping_private = [
                     ("2005,100d","0028,1052"), # Philips Intercept -> Rescale Intercept
                     ("2005,100e","0028,1053"), # Philips Slope -> Rescale Slope
    ]
...
def prepareInput(instancedict, headers_only, do_transpose=True, logTag="[prepareInput] ", skip_check=False, splitOnPosition=True, rgbchannel=US_RGB_USE_CHANNEL):
    ...
    if len(instancedict) == 1:
                ...
                if modality == 'CT' or modality == 'MR':
                    if not "RescaleIntercept" in dcmInfile: # in wrong place define for some MR files
                        ...
                        if dcmInfile.RescaleIntercept == "":
                            # try private tag
                            dcmInfile.RescaleIntercept = readDICOMtag(keymapping_private[0][0],dcmInfile,0)
                            if dcmInfile.RescaleIntercept == "":
                                dcmInfile.RescaleIntercept = 0
                        dcmInfile.RescaleSlope = readDICOMtag(keymapping[1][0],dcmInfile,0)
                        if dcmInfile.RescaleSlope == "":
                            # try private tag
                            dcmInfile.RescaleSlope = readDICOMtag(keymapping_private[1][0],dcmInfile,0)
                            if dcmInfile.RescaleSlope == "":
        ...
        try:
            dcmInfile = dcmseries.read_files(instancedict, True, readPixelData=False, splitOnPosition=splitOnPosition)[0]
                ...
                if modality == 'CT' or modality == 'MR':
                    for i in range(len(dcmInfile._datasets)):
                        if not "RescaleIntercept" in dcmInfile._datasets[i]: # in wrong place define for some MR files
                            ...
                            if dcmInfile._datasets[i].RescaleIntercept == "":
                                # try private tag
                                dcmInfile._datasets[i].RescaleIntercept = readDICOMtag(keymapping_private[0][0],dcmInfile,i)
                                if dcmInfile._datasets[i].RescaleIntercept == "":
                                    dcmInfile._datasets[i].RescaleIntercept = 0
                                    
                            dcmInfile._datasets[i].RescaleSlope = readDICOMtag(keymapping[1][0],dcmInfile,i)
                            if dcmInfile._datasets[i].RescaleSlope == "":
                                # try private tag
                                dcmInfile._datasets[i].RescaleSlope = readDICOMtag(keymapping_private[1][0],dcmInfile,i)
                                if dcmInfile._datasets[i].RescaleSlope == "":
                                    dcmInfile._datasets[i].RescaleSlope = 0
                pixeldataIn = dcmInfile.get_pixel_array()

        except Exception as e:
            raise ValueError("{} ERROR! {} is not a valid non-Enhanced DICOM series ({})".format(logTag, os.path.dirname(instancedict[0]), str(e)))
